...
read answ
if [[ "$answ" == "" ]]; then
    pacman -S cbatticon networkmanager network-manager-applet
fi
...
pacman --noconfirm -S ffmpeg imagemagick arandr mpv sxiv \
    qutebrowser firefox zathura zathura-pdf-mupdf \
    vifm highlight handbrake \
...
cd yay; makepkg -si
yay --useask -S mp3gain jmtpfs tesseract-git pdfjs mons chromium-widevine xmacro dragon-drag-and-drop-git lightdm-webkit-theme-aether picom-jonaburg-git
