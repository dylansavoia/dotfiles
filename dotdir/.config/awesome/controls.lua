...
local function toggle_show_all()
    ...
    if not show_all then
        for s in screen do curr_tags[s.index]=s.selected_tag end
...
local function change_layout(idx)
    ...
    naughty.notification({
        title = "Tag Layout",
        message = "",
        icon = ThemesPath..'default/layouts/'..layout..'w.png'
    })
end
...
keymapping.global = gears.table.join(
    ...
    awful.key({ Modkey, }, "g", function ()
        local scr = awful.screen.focused() 
        if scr.selected_tag.gap == dpi(1) then scr.selected_tag.gap = beautiful.useless_gap
        else scr.selected_tag.gap = dpi(1) end
    end,
    ...
    awful.key({ Modkey, "Shift"   }, "m", function ()
        ...
        {description = dsep.."Select Prev. Layout", group = "Layout"}
    ),
...
keymapping.client = gears.table.join(
    ...
    awful.key({ Modkey, "Shift" }, "s", function (c)
            awful.spawn.easy_async("rofi -window-command 'echo {window}' -show window",
            function (out)
        ...
        {description = dsep.."Put Next To Client", group = "Client"}
    ),
